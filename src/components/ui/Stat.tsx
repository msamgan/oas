...
function StatLabel({ className, children, ...rest }: StatLabelProps) {
    return (
        <div
            className={cn(
                'stat-label text-muted hover:text-text text-[15px] font-semibold tracking-[0.8px] uppercase transition-colors duration-300 ease-out',
                className,
            )}
            {...rest}
        >
