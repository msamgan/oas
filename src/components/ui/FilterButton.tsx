...
function FilterButton({ children, className, isActive = false, animationDelay, ...rest }: FilterButtonProps) {
    ...
    const stateClasses = isActive
        ? 'bg-linear-to-r from-accent to-[#ff6a00] text-white shadow-[0_4px_12px_rgba(255,122,24,0.3)] scale-105'
        : 'bg-black/4 text-muted border border-black/10 hover:bg-black/8 hover:text-text hover:border-black/20 hover:scale-105'
